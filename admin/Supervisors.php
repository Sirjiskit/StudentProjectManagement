...
<head>
            ...
            <div class="row">
                <div class="span7">
                    <header class="page-header">
                        <h3><i class="icon-user"></i>SUPERVISOR'S
                        </h3>
                    </header>
                </div>
    ...
    <section class="page container">
		...
		$(document).ready(function(){
			$('#li-Supervisors').parent('li').addClass("active");
			$.post('api/api.process?ListOfSupervisors',function(html){
			...
			$('#dpm').change(function(){
				...
				$('#dpm_name').html("&nbsp; "+arr[s][d]+" supervisors");
				$.post('api/api.process?ListOfSupervisors',{dpm:dpm},function(html){
				$('.studentList').html(html);
			});
			});
			$('#txtSearch').keyup(function(){
				...
				var dpm = $('#dpm').val();
				if(dpm==''){
					$.post('api/api.process?ListOfSupervisors&txtsearch='+txtsearch,function(html){
						$('.studentList').html(html);
					});
					return false;
				}
				$.post('api/api.process?ListOfSupervisors&txtsearch='+txtsearch,{dpm:dpm},function(html){
				$('.studentList').html(html);
				});
			});
		});
		function paggingcontrol(pageNum,totalRows){
			var dpm = $('#dpm').val();
			//alert(dpm);
	 		$.post('api/api.process?ListOfSupervisors&start='+pageNum+'&totalRows='+totalRows,{dpm:dpm},function(html){
				//alert(html);
				$('.studentList').html(html);
			});
		}
